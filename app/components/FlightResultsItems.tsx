...
const FlightResultsItems: React.FC<SearchResultItemProps> = ({ flights, maxPrice, maxDuration, layovers, currencyExchangeRate, currency }) => {
  ...
  const renderFlightPanels = (flightData: FlightSearchData['best_flights' | 'other_flights'], isBestFlights: boolean) => {
    return flightData
            ...
            <div className="flex flex-col items-center justify-center">
              <p>
                ...
                {currencyExchangeRate?.conversion_rates 
                  ? Math.floor(flight.price * currencyExchangeRate?.conversion_rates[currency])
                  : flight.price} 
                {currency}
              </p>
  ...
  return (
    <div className="search-result-item flex flex-col items-center justify-center text-primary-content font-bold ">
      <h2 className="text-6xl font-bold">Best Flights</h2>
      {renderFlightPanels(flights.best_flights, true)}
      <h2 className="text-6xl font-bold">Other Flights</h2>
      {renderFlightPanels(flights.other_flights, false)}
    </div>
  );
};

export default FlightResultsItems;
